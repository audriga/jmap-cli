...
class ChangesCalendarEventCommand extends BaseCommand {
  @override
  final name = 'changes';

  @override
  List<String> get aliases => ['calendarEvent.changes'];

  @override
  final description = 'Get change information of calendar events as JSON';

  ChangesCalendarEventCommand() {
    argParser.addOption(
      'state',
      help:
  ...
  Future<int> run() async {
    try {
      ...
      if (stateArg != null && stateArg.trim().isNotEmpty) {
        ...
        final getResp = await CalendarEventUtil.getCalendarEvents(
          client: httpClient,
      ...
      final changes = await CalendarEventUtil.changesCalendarEvents(
        client: httpClient,
        accountId: AccountId(Id(accountId)),
